...
```bash
$ cd ~/hands-on-with-anvil/challenges/Python_QML_Basics
...
Next, we will load the recommended GPU modules and the conda module:
...
```bash
$ which python3
/anvil/projects/x-cis230270/data/envs/qml-anvil/bin/python3
```
...
Additionally, we setup for `DDP` to split the calculations into [GPU] pieces, so the print will display "cuda:[#0-7]" for which GPU analyzed that particular part of the data. With the way we are running our script on Anvil (1 GPU per MPI rank), each process only sees their own "cuda:0" w/ GPU ID 0, even if you run with 4 GPUs per node.

```python
def train_model(rank, world_size): 
    torch.cuda.set_device(0) #assuming 1 gpu per MPI rank on Anvil
...
The `__main__` part of the code is what sets up the initial parallel environment, like the number of MPI ranks, the master port and address for our Slurm job, etc. This is necessary to set up proper communication between tasks on Anvil (especially when using multiple nodes). The rest of the `main` function prints out the GPUs being used so that we can analyze the comparisons between GPUs at the end of the testing. The last part that is run is a function called `setup`, which we defined at the very top of the "Functions" section above.
...
0. Make sure you copy over the scripts and are in your `/anvil/scratch/your_username/qml_test` directory:

    ```bash
    $ cd $SCRATCH
    $ mkdir qml_test
    $ cd qml_test
    $ cp ~/hands-on-with-anvil/challenges/Python_QML_Basics/qml.py ./qml.py
    $ cp ~/hands-on-with-anvil/challenges/Python_QML_Basics/submit_qml.sbatch ./submit_qml.sbatch
    ```

1. Use your favorite editor to change the integer following `-n` in `submit_qml.sbatch` to distribute the network over a specific number of tasks (pick an integer in the range from 1 to 4):
    ...
    ```bash
    srun -n1 python3 -W ignore -u qml.py
    ```
...
conda create -p /anvil/projects/x-cis230270/data/envs/qml-anvil python=3.10 -c conda-forge
source activate /anvil/projects/x-cis230270/data/envs/qml-anvil
